...
INSERT INTO p_companies (company_uuid, company_name, company_address, company_type, user_id, hub_id)
VALUES
    -- SUPPLY Companies
    (gen_random_uuid(), 'Supply Company A', '123 Main Street', 'SUPPLY', (SELECT user_id FROM p_users WHERE username = 'company1'), 1),
    (gen_random_uuid(), 'Supply Company B', '456 Oak Avenue', 'SUPPLY', (SELECT user_id FROM p_users WHERE username = 'company2'), 2),
    (gen_random_uuid(), 'Supply Company C', '789 Pine Road', 'SUPPLY', (SELECT user_id FROM p_users WHERE username = 'company3'), 3),

    -- DEMAND Companies
    (gen_random_uuid(), '스타벅스 가든파이브점', '서울 송파구 충민로 10 (문정동) 가든파이브툴', 'DEMAND', (SELECT user_id FROM p_users WHERE username = 'company4'), 1),
    (gen_random_uuid(), '스타벅스 고양원흥DT점', '경기 고양시 덕양구 서오릉로663번길 8 (원흥동)', 'DEMAND', (SELECT user_id FROM p_users WHERE username = 'company5'), 2),
    (gen_random_uuid(), 'CU 백암가창점', '경기 용인시 처인구 백암면 덕평로 168', 'DEMAND', (SELECT user_id FROM p_users WHERE username = 'company6'), 3),
    (gen_random_uuid(), '부산항 힐링야영장', '부산 동구 초량동 1185-1', 'DEMAND', (SELECT user_id FROM p_users WHERE username = 'company7'), 4),
    (gen_random_uuid(), '국채보상운동기념도서관', '대구 중구 공평로10길 25', 'DEMAND', (SELECT user_id FROM p_users WHERE username = 'company8'), 5),
    (gen_random_uuid(), '홈플러스 구월점', '인천 남동구 예술로 198', 'DEMAND', (SELECT user_id FROM p_users WHERE username = 'company9'), 6),
    (gen_random_uuid(), '상무조각공원', '광주 서구 상무시민로 3 세계광엑스포주제관', 'DEMAND', (SELECT user_id FROM p_users WHERE username = 'company10'), 7),
    (gen_random_uuid(), '스타벅스 대전대덕로점', '대전 서구 대덕대로234번길 18 (둔산동)', 'DEMAND', (SELECT user_id FROM p_users WHERE username = 'company11'), 8),
    (gen_random_uuid(), '옥동초등학교', '울산 남구 문수로 401', 'DEMAND', (SELECT user_id FROM p_users WHERE username = 'company12'), 9),
    (gen_random_uuid(), '물빛광장문화공원', '세종 소담동 510-1', 'DEMAND', (SELECT user_id FROM p_users WHERE username = 'company13'), 10),
    (gen_random_uuid(), '레고랜드 코리아 리조트', '강원 춘천시 하중도길 128 레고랜드 코리아 리조트', 'DEMAND', (SELECT user_id FROM p_users WHERE username = 'company14'), 11),
    (gen_random_uuid(), 'CGV 청주서문', '충북 청주시 상당구 상당로81번길 63', 'DEMAND', (SELECT user_id FROM p_users WHERE username = 'company15'), 12),
    (gen_random_uuid(), '용봉보건진료소', '충남 홍성군 홍북읍 용봉산3길 136', 'DEMAND', (SELECT user_id FROM p_users WHERE username = 'company16'), 13),
    (gen_random_uuid(), '롯데마트 전주점', '전북 전주시 완산구 우전로 240 롯데마트 전주점', 'DEMAND', (SELECT user_id FROM p_users WHERE username = 'company17'), 14),
